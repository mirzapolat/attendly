...
const AccountSettingsDialog = ({ open, onOpenChange }: AccountSettingsDialogProps) => {
  ...
  useEffect(() => {
    if (!open || authLoading || !user?.id) return;
    ...
    void fetchProfile();
  }, [open, authLoading, user?.id, user?.email, toast]);
  ...
  }, [open, authLoading, user, onOpenChange, navigate]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="lg:max-w-2xl lg:max-h-[90vh]">
        <DialogHeader>
        ...
        {loading ? (
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <section className="rounded-2xl border border-destructive/40 bg-destructive/5 p-4 sm:p-5 space-y-3">
              <h3 className="text-base font-semibold text-destructive flex items-center gap-2">
                <Trash2 className="w-4 h-4" />
                Danger Zone
              </h3>
              <p className="text-sm text-muted-foreground">
                Once you delete your account, there is no going back. All your owned workspaces, events, series, and attendance records will be permanently removed.
              </p>
              <Button
                type="button"
                variant="destructive"
